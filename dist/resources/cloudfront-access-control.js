 ...
 */
export class CloudfrontAccessControl extends cdk.Resource {
    functionRef;
    functionName;
    functionArn;
 ...
 */
function cidrs2pattern(cidrs) {
    const byver = { 4: [], 6: [] };
    cidrs.forEach((cidr) => byver[ipv(cidr)].push(cidr2bin(cidr)));
    const pattern = { 4: null, 6: null };
    for (const ver of [4, 6]) {
        if (!byver[ver].length)
            continue;
        const re = byver[ver]
            .sort((a, b) => a.length - b.length)
            .reduce((carry, cidr) => (carry.some((v) => cidr.startsWith(v)) ? carry : carry.concat(cidr)), [])
            .join('|');
        pattern[ver] = `^(?:${re})`;
    }
    return pattern;
}
//# sourceMappingURL=cloudfront-access-control.js.map