...
export class EdgeFunction extends Construct implements iam.IGrantable {
  private readonly body: EdgeFunctionBody;
  private _policy?: iam.Policy;
...
class CleanupEdgeFunctions extends Construct {
  ...
  static of(scope: Construct): CleanupEdgeFunctions {
    const scopeStack = cdk.Stack.of(scope);
    return (
      (scopeStack.node.tryFindChild('CleanupEdgeFunctions') as CleanupEdgeFunctions) ??
  ...
  addEdgeFunction(handler: lambda.Function) {
    this.functions.push({
